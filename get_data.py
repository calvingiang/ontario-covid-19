...
def get_ontario_data():
    ...
    path_url = '/api/3/action/datastore_search?&resource_id=455fd63b-603d-4608-8216-7d8647f43350&limit=1000000'
...
def get_ontario_cases():
    ...
    path_url = '/api/3/action/datastore_search?&resource_id=ed270bb8-340b-41f9-a7c6-e8ef587e6d11&limit=1000000'
    url = base_url + path_url
    covid_request = requests.get(url)
    records = covid_request.json()['result']['records']
    new_df = pandas.DataFrame(records)
...
def get_cases_only():
    ...
    cases_df = covid_cases[['Reported Date', 'Total Cases']]
    cases_df = cases_df[['Reported Date','Total Cases']].fillna(0).reset_index().drop(columns=['index'])
